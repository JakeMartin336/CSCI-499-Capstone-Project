...
function toggleBuddyButton() {
    // Check if the user marked as interested or going
    const goingRadio = document.getElementById("goingRadio");
    const interestedRadio = document.getElementById("interestedRadio");

    const buddyButton = document.querySelector('.buddy_button');

    if (goingRadio.checked || interestedRadio.checked) {
        buddyButton.style.display = 'block'; // Show the button
    } else {
        buddyButton.style.display = 'none'; // Hide the button
    }
}
